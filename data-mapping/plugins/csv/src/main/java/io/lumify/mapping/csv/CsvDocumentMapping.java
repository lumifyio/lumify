...
public class CsvDocumentMapping extends AbstractColumnDocumentMapping {
    ...
    @Override
    protected Iterable<Row> getRows(final InputStream input) {
    ...
    private class CsvRowIterator implements Iterator<Row> {
         ...
         */
        public CsvRowIterator(final Reader reader) {
            try {
                records = CSVFormat.EXCEL.parse(reader).iterator();
            } catch (IOException ioe) {
                throw new LumifyDataMappingException("Error reading rows from CSV", ioe);
            }
            // skip configured lines
            for (int ln=0; ln < skipRows && records.hasNext(); ln++) {
                records.next();
            }
        }
