 ...
 * by providing a mapping for the empty string.
 */
@JsonTypeName("mappedMultiColumn")
public class MappedMultiColumnValue implements ColumnValue<String> {
    /**
    ...
    public MappedMultiColumnValue(@JsonProperty("keyColumns") final List<ColumnValue<String>> keyColumns,
        ...
        checkArgument(separator == null || !separator.trim().isEmpty(), "separator must be non-empty if provided");
    ...
    private String buildKey(final List<String> row, final int length) {
    ...
    public int compareTo(final ColumnValue<?> o) {
        return keyColumns.get(0).compareTo(o);
    }
}
