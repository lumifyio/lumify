...
public class BaseOntologyTest {
    ...
    public void testDefineOntology() {
        when(ontologyRepository.getOrCreateConcept((Concept) isNull(), eq(OntologyRepository.ROOT_CONCEPT_IRI), eq("root"))).thenReturn(rootConcept);
        when(ontologyRepository.getOrCreateConcept(eq(rootConcept), eq(OntologyRepository.ENTITY_CONCEPT_IRI), eq("thing"))).thenReturn(entityConcept);
    ...
    public void testIsOntologyDefinedExceptionWithFalse() {
        when(ontologyRepository.getConceptById(OntologyRepository.ROOT_CONCEPT_IRI)).thenThrow(new RuntimeException(OntologyLumifyProperties.ONTOLOGY_TITLE.getKey(), new Throwable("testing exception")));
