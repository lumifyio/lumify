    ...
    public GraphPropertyWorkResult execute(InputStream in, GraphPropertyWorkData data) throws Exception {
        ...
        try {
            ...
            try {
                ...
                m.save();
                graph.flush();
                workQueueRepository.pushGraphPropertyQueue(data.getVertex().getId(), PROPERTY_KEY, getAudioProperty(AUDIO_TYPE_OGG).getKey());
                workQueueRepository.pushGraphPropertyQueue(data.getVertex().getId(), PROPERTY_KEY, getAudioSizeProperty(AUDIO_TYPE_OGG).getKey());
            } finally {
                mp4FileIn.close();
            }
