...
public class WorkspaceUndoIntegrationTest extends TestBase {
    ...
    private void importArtifact() throws IOException, ApiException {
        ...
        lumifyApi.getVertexApi().setProperty(susanFengVertex.getId(), TEST_MULTI_VALUE_KEY, LumifyProperties.TITLE.getPropertyName(), "Susan Feng", "", "test");

        ClientApiElement jeffKunkleVertex = lumifyApi.getVertexApi().create(CONCEPT_TEST_PERSON, "");
        lumifyApi.getVertexApi().setProperty(jeffKunkleVertex.getId(), TEST_MULTI_VALUE_KEY, LumifyProperties.TITLE.getPropertyName(), "Jeff Kunkle", "", "test");

        ClientApiEdgeWithVertexData e = lumifyApi.getEdgeApi().create(susanFengVertex.getId(), jeffKunkleVertex.getId(), TestOntology.WORKS_FOR, "");
        lumifyApi.getEdgeApi().setProperty(e.getId(), "key1", "http://lumify.io/test#firstName", "edge property value", "", "");
